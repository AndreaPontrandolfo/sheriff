...
ESLint provides a very powerful rule called `no-restricted-syntax` ([docs](https://eslint.org/docs/latest/rules/no-restricted-syntax)). It accepts an array of objects. Each object represent a specific Javascript syntax feature that you may want to opt-out.
...
You provide your own `no-restricted-syntax` rule. You can do this as you normally would, appending the rule to the `FlatConfig` array, after Sheriff.
...
Conveniently, Sheriff exports it's own `no-restricted-syntax` entry as the `baseNoRestrictedSyntaxRules` array.
To shrink the Sheriff entry of `no-restricted-syntax`, you can use the Javascript `toSpliced()` method.
