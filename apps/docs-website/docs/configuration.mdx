...
ESLint provides a very powerful rule called [`no-restricted-syntax`](https://eslint.org/docs/latest/rules/no-restricted-syntax). It accepts an array of objects. Each object represent a specific Javascript syntax feature that you may want to forbid.
...
```js title="eslint.config.js"
import { baseNoRestrictedSyntaxRules, sheriff, tseslint } from "eslint-config-sheriff";
...
```ts title="eslint.config.ts"
import { baseNoRestrictedSyntaxRules, sheriff, type SheriffSettings, tseslint } from "eslint-config-sheriff";
