...
For example, let's say you want to disable a Sheriff rule, like `import-x/first`:
...
export default tseslint.config(sheriff(sheriffOptions),
  {
    rules: {
      // highlight-next-line
      "import-x/first": 0, // 'import-x/first' is now disabled everywhere.
    },
  },
);
```
...
export default tseslint.config(sheriff(sheriffOptions),
  {
    rules: {
      // highlight-next-line
      "import-x/first": 2, // 'import-x/first' is now enabled everywhere.
    },
  },
);
```
